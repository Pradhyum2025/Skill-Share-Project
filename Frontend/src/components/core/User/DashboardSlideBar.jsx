...
export default function Dashbord({setIsSlideBarOpen,handleSelectSubSection}) {
  ...
  //sign out handler
  const handleSignOut = () => {
    return signOut(dispatch, navigate);
  }
  return (
    <>
      {/*---------------- Aside slidebar ---------------- */}
          ...
          <ul class="space-y-2 font-medium">
            ...
            >
              ...
              <div class="flex items-center font-bold p-2 text-indigo-300">
                ...
                <span class="ms-3">{currUser.accountType + " " + 'pannel'}</span>
              </div>
            </li>

            {/*------- My Course for student and instructor OR My category for admin ------- */}
            {(currUser.accountType !== 'undefined' && currUser.accountType === 'Admin') ?
              <Link 
            ...
            {currUser.accountType !== 'undefined' && currUser.accountType === 'Instructor' ?
              <Link 
              onClick={handleSelectSubSection}
              to='/dashbord/createCourse'>
                <li>
                  <a href="#Profile" class={`${currLocation==='/dashbord/createCourse'?'dark:bg-gray-700':''} flex items-center p-2 text-gray-900 rounded-lg dark:text-white `}>
                    <MdCreateNewFolder className={`${currLocation==='/dashbord/createCourse'?'text-white':'text-gray-400'}  flex-shrink-0 w-5 h-7  transition duration-75`} />
                    <span class="flex-1 ms-3 whitespace-nowrap">Create course</span>
                  </a>
                </li>
              </Link> :
              // Create catagory for Admin
              (currUser.accountType !== 'undefined' && currUser.accountType === 'Admin') ?
                <Link 
                onClick={handleSelectSubSection}
                to='/dashbord/createCategory'>
                  <li>
                    <a href="#Profile" class={`${currLocation==='/dashbord/createCategory'?'dark:bg-gray-700':''} flex items-center p-2 text-gray-900 rounded-lg dark:text-white  group`}>
                      <MdCreateNewFolder className={`${currLocation==='/dashbord/createCategory'?'text-white':''}  flex-shrink-0 w-5 h-5 text-gray-400 transition duration-75`} />
             ...
             to='/dashbord/instructor'>
               <li>
                 <a href="#Profile" class={`${currLocation==='/dashbord/instructor'?'dark:bg-gray-700':''} flex items-center p-2 text-gray-900 rounded-lg dark:text-white  group`}>
                   <GiTeacher className={`${currLocation==='/dashbord/instructor'?'text-white':''}  flex-shrink-0 w-4 h-4 text-gray-400 transition duration-75`} />
                   <span class="flex-1 ms-3 whitespace-nowrap">Instructor</span>
                 </a>
               </li>
             </Link> 
            :
            ...
            {currUser?.accountType === 'Student' ?
              ...
              to={'/dashbord/cart'}>
                <li>
                  <a href="#Card" class={`${currLocation==='/dashbord/cart'?'dark:bg-gray-700':''} flex items-center p-2 text-gray-900 rounded-lg dark:text-white  group`}>
                    <svg class={`${currLocation==='/dashbord/cart'?'text-white':''}  flex-shrink-0 w-5 h-5 text-gray-400 transition duration-75`}
