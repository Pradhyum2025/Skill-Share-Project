...
import { deleteSubSection } from '../../../../operations/subSection';

export default function DeleteSubSectionModal({courseId,sectionId,subSection}) {
  const currUser = useSelector(store=>store.auth);
  const dispatch = useDispatch();

  // dispatch,courseId,sectionId,subSectionId,token
  const handleDeleteSection= ()=>{
    if(currUser.token && courseId && sectionId && subSection){
      deleteSubSection(dispatch,courseId,sectionId,subSection._id,currUser.token)
    }else{
      return
    }
  }
      ...
      <div className="modal-box bg-white">
        ...
        <button
          ...
          <div class="mb-2 text-center">
            ...
            <p className='text-indigo-600 font-[700] text-[1.2rem]'>{subSection.title}</p>
