...
import { createSubSection } from "../../../../operations/subSection";
...
export const CreateSubSection = () => {
  ...
  const sectionName = location.state?.message
  ...
  const onSubmit = async (data) => {
    if (courseId && sectionId && currUser.token) {
      createSubSection(dispatch, navigate, data, courseId, sectionId, currUser.token)
    } else {
      return;
    }
  };
  ...
  const handleBackNaviaget=()=>{
    if(courseId){
      return navigate(`/dashbord/show/${courseId}`)  
    }else{
      return ;
    }
  }
  return (
    <div className="w-full min-h-screen bg-gray-50 py-3 px-3">
      ...
      onClick={handleBackNaviaget}
       className={`btn flex items-center text-white mb-4 group transition-colors duration-200 bg-indigo-600 border-0 hover:bg-indigo-700 text-[.97rem] disabled:bg-indigo-600 disabled:cursor-not-allowed disabled:text-white`}>
      ...
      <div className="bg-white shadow-md rounded-lg p-6 max-w-3xl mx-auto">
        <p className="text-2xl font-bold text-indigo-600 mb-2 text-center"> {sectionName}</p>
        <h2
          className="text-lg font-semibold text-gray-700 mb-6 text-center">Create Sub-Section</h2>
